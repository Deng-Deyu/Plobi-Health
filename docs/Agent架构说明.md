...
本项目在 5 小时黑客松时限内采用了一系列务实的工程兜底策略，确保核心流程在任何异常情况下都能继续运行：

- **失效兜底机制**：LLM JSON 解析失败时，系统自动提取 markdown 代码块中的 JSON；若仍失败，则基于文本分句生成基础知识点作为 fallback，保证 demo 流程不中断。Embedding API 限流时按 10 条分批并自动重试，避免一次性请求过多导致服务不可用。
- **localStorage 历史持久化**：对话历史和整合决策通过 localStorage 双写持久化，刷新页面后用户状态不丢失。同时 `/tmp/aligned.json` 在服务端保存整合结果，实现前后端双重保障。

这些机制在极限开发时间内有效平衡了系统鲁棒性与实现成本，但也意味着对边缘 case 的处理较为粗糙（如空知识点的语义质量有限）。

### 10.2 未来改进规划

1. **增加压缩比桑基图**：将整合前后的知识点流动可视化，直观展示哪些知识点被合并、保留或删除，提升报告的可读性。
2. **知识冲突检测**：当多本教材对同一知识点的描述存在矛盾时，自动标记冲突区域并提示用户人工裁决，而非简单合并。
3. **外部持久化存储**：将 `/tmp/*.json` 文件持久化方案替换为 Redis 或轻量级数据库，解决 Vercel Serverless 冷启动后数据丢失的问题。
